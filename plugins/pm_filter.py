...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("❌ 𝙳𝙰 𝙼𝚆𝙾𝙽𝚄𝚂𝙴 𝙸𝚃𝙷𝙴 𝙽𝙸𝙽𝙰𝙺𝙴 𝚄𝙻𝙻𝙰𝚃𝙷𝙴 𝙰𝙻𝙻𝙰 𝙺𝙴𝚃𝚃𝙾 ❌", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def advantage_spoll_choker(bot, query):
    ...
    if int(user) != 0 and query.from_user.id != int(user):
        return await query.answer("❌ 𝙳𝙰 𝙼𝚆𝙾𝙽𝚄𝚂𝙴 𝙸𝚃𝙷𝙴 𝙽𝙸𝙽𝙰𝙺𝙴 𝚄𝙻𝙻𝙰𝚃𝙴 𝙰𝙻𝙻𝙰 𝙺𝙴𝚃𝚃𝙾 ❌", show_alert=True)
